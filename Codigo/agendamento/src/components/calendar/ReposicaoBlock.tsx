...
interface ReposicaoBlockProps {
  reposicao: ReposicaoDTO;
  hourStart: number;
  onClick?: (e: React.MouseEvent) => void;
}

export function ReposicaoBlock({ reposicao, hourStart, onClick }: ReposicaoBlockProps) {
  const { t, lang } = useLanguage();
  const top = ((timeToMinutes(reposicao.horario) - hourStart * 60) / 60) * CELL_HEIGHT;
  const height = Math.max((50 / 60) * CELL_HEIGHT - 2, 24);
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'border border-dashed border-amber-400 bg-[color-mix(in_srgb,#f59e0b_10%,var(--surface))]',
        onClick ? 'cursor-pointer hover:z-30' : 'cursor-default',
      )}
      style={{ top: `${top}px`, height: `${height}px` }}
    >
      <div className="absolute left-0 inset-y-0 w-[3px] bg-amber-500" />
      <div className="flex items-center gap-1 pl-2 pr-1 h-full min-w-0">
        <RefreshCw size={9} className="shrink-0 text-amber-600 opacity-70" />
        <p className="text-[10px] font-semibold truncate text-amber-700 leading-tight">{t('calendar.makeUp')}</p>
        {height > 30 && count > 0 && <span className="ml-auto shrink-0 text-[9px] font-medium text-(--muted)">{count}↑</span>}
      </div>
