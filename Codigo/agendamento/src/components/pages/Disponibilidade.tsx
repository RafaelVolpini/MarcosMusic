...
/** Retorna as datas ISO de in├¡cio e fim da semana atual (segÔÇôdom). */
...
export function RoomsPage({ availability, availabilityReposicao, lessons, onChangeAvailability, onChangeAvailabilityReposicao }: RoomsPageProps) {
  ...
  // ÔöÇÔöÇ Converte DTOs do banco em WeeklyAvailability ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  const applyDTOs = useCallback((dtos: DisponibilidadeResponseDTO[]) => {
    ...
    for (const dto of dtos) {
      ...
      if (!avail[day]) continue; // dia desconhecido ÔÇö ignora
      // aulaMarcada tamb├®m entra em avail para evitar badge CONFLITO no calend├írio
  ...
  // ÔöÇÔöÇ Carrega tudo em sequ├¬ncia: aulas desta semana ÔåÆ disponibilidade ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      // 2. Disponibilidade (configura├º├úo semanal do professor)
      const dtos = await buscarDisponibilidade();
      applyDTOs(dtos);
    } catch {
      // fallback: mant├®m estado atual
    } finally {
      setLoadingDb(false);
    }
  }, [applyDTOs]);
  ...
  const effectiveLessons = useMemo(() => {
    ...
    for (const l of apiLessons) byId.set(l.id, l); // apiLessons tem preced├¬ncia (mais atualizado)
    return [...byId.values()];
  }, [lessons, apiLessons]);
  ...
  // Ciclo 3 estados: indispon├¡vel ÔåÆ dispon├¡vel ÔåÆ reposi├º├úo ÔåÆ indispon├¡vel
  const toggleSlot = (day: DayKey, time: string) => {
    ...
    if (!isAvail && !isReposicao) {
      ...
      // reposi├º├úo ÔåÆ indispon├¡vel
      if (hasScheduled) {
        setWarning('Esse hor├írio possui aula agendada. Reagende ou cancele antes de tornar indispon├¡vel.');
        return;
      }
  ...
  const clearAll = () => {
    ...
    setWarning('Hor├írios com aulas agendadas foram mantidos.');
    setSaveStatus('idle');
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--muted)] mt-0.5">
            Clique nos hor├írios para marcar ou remover disponibilidade para aulas
          </p>
        </div>
      </div>

      {/* ÔöÇÔöÇ Warning ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      {warning && (
      ...
      {/* ÔöÇÔöÇ Legend + Actions ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      <div className="flex items-center justify-between gap-4 mb-5 flex-wrap">
        {/* Legenda ÔÇö lado esquerdo */}
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button variant="secondary" size="sm" onClick={applyBusinessHours}>
            <Plus size={14} /> Hor├írio comercial
          </Button>
          ...
          >
            <Save size={14} />
            {saveStatus === 'saving' ? 'SalvandoÔÇª' : saveStatus === 'saved' ? 'Ô£ô Salvo!' : saveStatus === 'error' ? 'Erro ao salvar' : 'Salvar'}
          </Button>
        </div>
      </div>

      {/* ÔöÇÔöÇ Day cards ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
                ...
                {TIME_SLOTS.map((time) => {
                  ...
                  return (
                    <button
                      ...
                      title={
                        hasLesson
                          ? `${time} ÔÇö ${lessonCount} aula(s) agendada(s) ÔÇö n├úo pode ser removido`
                          : isActive
                          ? `${time} ÔÇö dispon├¡vel (clique para reposi├º├úo)`
                          : isReposicao
                          ? `${time} ÔÇö reposi├º├úo (clique para bloquear)`
                          : `${time} ÔÇö indispon├¡vel (clique para liberar)`
                      }
                      className={cn(
      ...
      {/* ÔöÇÔöÇ Footer ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ */}
      <div
      ...
      >
        <div className="flex items-center gap-2 text-[var(--muted)]">
          ...
          Hor├írios indispon├¡veis ficam bloqueados no calend├írio de agendamento.
        </div>
