...
import { CalendarClock, Check, Clock3, RefreshCw, Save, Trash2 } from 'lucide-react';
...
export function RoomsPage({ availability, availabilityReposicao, onChangeAvailability, onChangeAvailabilityReposicao }: RoomsPageProps) {
  ...
  const totalReposicaoSlots = Object.values(availabilityReposicao).reduce((sum, slots) => sum + slots.length, 0);
  ...
  const clearAll = () => {
    const empty = (): WeeklyAvailability => ({ seg: [], ter: [], qua: [], qui: [], sex: [], sab: [], dom: [] });
    onChangeAvailability(empty());
    onChangeAvailabilityReposicao(empty());
    setWarning('');
    setSaveStatus('idle');
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-[var(--muted)] mt-0.5">
            Clique nos horários para alternar entre disponível, reposição e indisponível
          </p>
        </div>
      </div>
